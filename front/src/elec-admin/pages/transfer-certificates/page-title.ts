...
export function usePageTitle(state: CBQueryStates) {
  ...
  const statuses: any = {
    [ElecTransferCertificateStatus.Pending]: title + " " + t("en attente"),
    [ElecTransferCertificateStatus.Accepted]: title + " " + t("acceptée"),
    [ElecTransferCertificateStatus.Rejected]: title + " " + t("rejetée"),
  }
