      ...
      tabs={[
        {
          ...
          label: t("En attente ({{count}})", { count: snapshot.meter_readings_applications_pending }),
        },

        {
          key: ElecAdminAuditStatus.AuditInProgress,
          path: ElecAdminAuditStatus.AuditInProgress.toLowerCase(),
          label: t("En cours d'audit ({{count}})", { count: snapshot.meter_readings_applications_audit_in_progress })
        },
        {
          key: ElecAdminAuditStatus.AuditDone,
          path: ElecAdminAuditStatus.AuditDone.toLowerCase(),
          label: t("Audit à valider ({{count}})", { count: snapshot.meter_readings_applications_audit_done })
        },
        {
          key: ElecAdminAuditStatus.History,
          path: ElecAdminAuditStatus.History.toLowerCase(),
          label: t("Historique ({{count}})", { count: snapshot.meter_readings_applications_history })
        },
