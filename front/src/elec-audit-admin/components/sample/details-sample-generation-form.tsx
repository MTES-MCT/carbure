...
const SampleGenerationForm = ({
  ...
  const generateSampleMutation = useMutation(generateSampleQuery, {
    onSuccess(response) {
      ...
      if (!sample) {
        notify(t("L'échantillon n'a pas pu être généré !"), {
          variant: "warning",
        })
        return
      }
    ...
    onError(err) {
      notifyError(
        err,
        t("Une erreur est survene, l'échantillon n'a pas pu être généré.")
      )
    },
  })
  ...
  const buttonText = !retry
    ? t("Générer l'échantillon")
    : t("Générer un nouvel échantillon")
