  ...
  return (
    <>
      <main>
        ...
        <section>
          <p>
            <Trans
              defaults={
                "Votre fichier d'audit <b>{{ fileName }}</b> ne comporte aucune erreur. Les informations peuvent être transmises à la DGEC."
              }
              values={{ fileName }}
            />
          </p>
          {!!commentCount && (
            <Alert icon={Message} variant="info">
              <Trans
                defaults={
                  "Vous avez commentés <b>{{ count }} points de charges </b> dans ce fichier excel. Ils seront transmis à l'administration."
                }
      ...
      <footer>
        <Button
          icon={Send}
          label={t("Transmettre le résultat d'audit")}
          variant="primary"
          action={onAcceptFile}
          loading={acceptAuditReport.loading}
        />
        <Button
          icon={ChevronLeft}
          label={t("Précédent")}
