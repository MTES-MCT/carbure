...
export const ROUTE_URLS = {
  ...
  ACCOUNTING: (entity_id: number) => {
    ...
    return {
      ...
      OBJECTIVES: {
        ROOT: `${baseUrl}/objectives`,
        YEAR: (year: number) => `${baseUrl}/objectives/${year}`,
      },
      ADMIN: {
  ...
  HYDROGEN: (entity_id: number, year: number) => {
  ...
  AUTH: {
    ...
    ACTIVATE_REQUEST: "/auth/register#activate-request",
    LOGOUT: "/auth/logout",
    RESET_PASSWORD_REQUEST: "/auth/login#reset-password-request",
    RESET_PASSWORD_PENDING: "/auth/login#reset-password-pending",
