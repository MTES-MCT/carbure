...
export const DechetIndustrielAlert = ({
  ...
  return (
    <Notice
      variant={valid ? "info" : "alert"}
      icon={valid ? "ri-check-fill" : "ri-error-warning-line"}
      title={t("Spécificité Déchets industriels")}
    >
      {valid && (
        <p style={{ marginTop: "var(--spacing-xs)" }}>
          {t(
            "Le questionnaire a bien été renseigné. Vous pouvez encore le modifier depuis l'onglet Fichiers."
          )}
        </p>
      )}
      {!valid && (
