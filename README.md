...
```bash
docker compose exec carbure-django uv run python3 web/manage.py createsuperuser
```
...
Une fois les modèles ou champs modifiés, créer un fichier de migration par la commande `docker compose exec carbure-django uv run python3 web/manage.py makemigrations`

## Effectuer une migration

Appliquer les migrations sur la base de données par la commande `docker compose exec carbure-django uv run python3 web/manage.py migrate`
...
Pour lancer tous les tests d'un module : `docker compose run --rm carbure-test uv run test <nom_du_module>`

Pour lancer un seul test : `docker compose run --rm carbure-test uv run  test api.v5.saf.airline.tests.tests_ticket_details.SafTicketDetailsTest`
