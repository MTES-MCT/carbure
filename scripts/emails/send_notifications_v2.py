...
def main(args):
    ...
    for entity, oldest_notif_dt in sorted(entity_oldest_notif.items(), key=lambda x: x[1]):
        ...
        lots_tofix = [n for n in notifs if n.notif_type in [EmailNotification.CORRECTION_REQUEST, EmailNotification.LOT_REJECTED]]
        email_context = {
            'entity': entity,
            'nb_notifications': notifs.count(),
            'domain': domain,
            'lots_in': lots_in,
            'lots_in_suppliers': ','.join(set([n.linked_tx.carbure_vendor.name for n in lots_in])),
            'lots_fixed': lots_fixed,
            'lots_fixed_suppliers': ','.join(set([n.linked_tx.carbure_vendor.name for n in lots_fixed])),
            'lots_tofix': lots_tofix,
            'lots_tofix_clients': ','.join(set([n.linked_tx.carbure_client.name for n in lots_tofix])),
