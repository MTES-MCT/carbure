...
def check_volumes():
    ...
    for s in stocks:
        ...
        transfos = (
            CarbureStockTransformation.objects.filter(source_stock=s)
            .values("volume_deducted_from_source")
            .aggregate(sumvol=Sum("volume_deducted_from_source"))
        )
        sum_child = 0
        ...
        if s.parent_lot:
            ...
            if abs(diff) > 0.01:
                print(
                    "Stock %s theo remaining %.2f real remaining %.2f (id %d)"
                    % (s.carbure_client.name, theo_remaining_volume, s.remaining_volume, s.id)
                )
