...
if __name__ == '__main__':
    ...
    etbe_child = LotTransaction.objects.filter(lot__parent_lot__isnull=False, lot__biocarburant__code='ETBE', lot__parent_lot__biocarburant__code='ETH')
    for e in etbe_child:
    ...
    etbe_child = LotTransaction.objects.filter(lot__biocarburant__code='ETBE', lot__parent_lot__biocarburant__code='ETH').exclude(parent_tx__is_forwarded=True)
    for e in etbe_child:
        if e.id not in transformed_out:
            print('Found linked ETH-ETBE lots/transactions with no transformation. %s Parent lot %s Tx %s Parent Tx %s' % (e.lot.id, e.lot.parent_lot_id, e.id, e.parent_tx_id))
    ...
    for t in transformations:
        if t.new_stock.parent_tx_id != t.previous_stock.id:
            ...
            print('New stock %s parent tx %s previous stock %s' % (t.new_stock.id, t.new_stock.parent_tx_id, t.previous_stock.id))
        ...
        if new_stock_parent_lot_id != old_stock_lot_id:
            print('New Stock tx id %s Previous tx id %s New Stock Parent Tx id %s' % (t.new_stock.id, t.previous_stock.id, t.new_stock.parent_tx_id))
            print('New lot id %s previous %s new parent lot %s. Update with %s' % (t.new_stock.lot.id, t.previous_stock.lot.id, t.new_stock.lot.parent_lot_id, old_stock_lot_id))
            # update
