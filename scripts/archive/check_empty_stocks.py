...
def fix_etbe():
    ...
    for t in transformations:
        if t.previous_stock.id == t.new_stock.id:
            ...
            t.save()
            print('Fixed ETBE Transformation %d. Previous stock: %d New Stock %d' % (t.id, t.previous_stock.id, t.new_stock.id))
    ...
    for t in txs:
        try:
            ...
            child = LotTransaction.objects.filter(lot__parent_lot=initial_eth_stock_line.lot, lot__biocarburant__code='ETH', lot__status='Validated')
            for c in child:
            ...
            if abs(diff) > 0.1:
                print('Original Stock Line volume [%f] remaining [%f] theo remaining [%f] diff [%f]' % (initial_eth_stock_line.lot.volume, initial_eth_stock_line.lot.remaining_volume, theo_remaining, diff))
                print('ETH transformed into ETBE [%f], sent directly [%f]' % (total_volume_converted_to_etbe, total_volume_sent_as_eth))
