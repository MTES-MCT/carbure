...
def check_odd_forward():
    ...
    for l in lots:
        try:
            ...
            print('Missing child for txid %d. Period %s Sender %s Client %s' % (l.id, l.lot.period, l.carbure_vendor.name if l.carbure_vendor else l.lot.unknown_supplier, l.carbure_client.name if l.carbure_client else l.unknown_client))
            # check with lot
