...
def get_prefetched_data(entity=None):
    ...
    d["locked_years"] = [
        locked_year.year for locked_year in LockedYear.objects.filter(locked=True)
    ]
    ...
    if entity:
        ...
        d["my_production_sites"] = {
            ps.name.upper(): ps
            for ps in ProductionSite.objects.prefetch_related(
                "productionsiteinput_set",
                "productionsiteoutput_set",
                "productionsitecertificate_set",
            ).filter(producer=entity)
        }
        # get all my linked certificates
        d["my_vendor_certificates"] = [
            c.certificate.certificate_id
            for c in EntityCertificate.objects.filter(entity=entity)
        ]
    else:
        d["my_production_sites"] = {}
        d["my_vendor_certificates"] = []
    ...
    all_ps_inputs = ProductionSiteInput.objects.values(
        "production_site_id", "matiere_premiere_id"
    )
    all_ps_outputs = ProductionSiteOutput.objects.values(
        "production_site_id", "biocarburant_id"
    )
    ...
    for psite in list(all_ps):
        ...
        production_sites[psite.id]["feedstock_ids"] = feedstock_by_ps.get(
            psite.id, []
        )  # [mp['matiere_premiere_id'] for mp in psite.productionsiteinput_set.all().values('matiere_premiere_id')]
        production_sites[psite.id]["biofuel_ids"] = biofuel_by_ps.get(
            psite.id, []
    ...
    client_entities = list(
        Entity.objects.filter(
            entity_type__in=[Entity.PRODUCER, Entity.OPERATOR, Entity.TRADER]
        )
    )
    d["clients"] = {c.id: c for c in client_entities}
    d["clientsbyname"] = {c.name.upper(): c for c in client_entities}
    d["certificates"] = {
        c["certificate_id"].upper(): c
        for c in GenericCertificate.objects.filter(valid_until__gte=lastyear).values(
            "certificate_id", "valid_until"
        )
    }
    d["double_counting_certificates"] = {
        c.certificate_id: c for c in DoubleCountingRegistration.objects.all()
    }
    d["etd"] = {
        s.feedstock: s.default_value
        for s in ETDStats.objects.select_related("feedstock").all()
    }
    d["eec"] = {
        s.feedstock.code + s.origin.code_pays: s
        for s in EECStats.objects.select_related("feedstock", "origin").all()
    }
    d["ep"] = {
        s.feedstock.code + s.biofuel.code: s
        for s in EPStats.objects.select_related("feedstock", "biofuel").all()
    }
