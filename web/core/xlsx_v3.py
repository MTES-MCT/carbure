...
def make_dump_lots_sheet(workbook, entity, transactions, stocks=False):
    ...
    columns += ['volume', 'biocarburant_code', 'matiere_premiere_code', 'categorie_matiere_premiere', 'pays_origine_code',
               'eec', 'el', 'ep', 'etd', 'eu', 'esca', 'eccs', 'eccr', 'eee', 'ghg_total', 'ghg_reduction', 'ghg_reduction_red_ii',
...
def make_template_carbure_lots_sheet(workbook, entity):
    ...
    if entity.entity_type in [Entity.PRODUCER, Entity.TRADER]:
        ...
        # CASE 6
        rows.append(['mise a consommation', '', random.choice(psites).name, '', '', '', '', '', '', '', 5700, 'ETH', 'BETTERAVE', 'FR', random.randint(8, 13), random.randint(2, 5), random.randint(1, 3), random.randint(1, 2), float(random.randint(5, 30)) / 10.0, 0, 0, 0, 0, get_random_dae(), random.choice(clients).name, today, random.choice(delivery_sites).name, '', 'RFC'])
...
def make_template_carbure_stocks_sheet(workbook, entity):
    ...
    # header
    bold = workbook.add_format({'bold': True})
    columns = ['champ_libre', 'carbure_stock_id', 'volume', 'dae', 'vendor_certificate', 'client', 'delivery_date', 'delivery_site', 'delivery_site_country']
    for i, c in enumerate(columns):
        worksheet_lots.write(0, i, c, bold)
