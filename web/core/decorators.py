...
def check_admin_rights(external_rights=None, user_rights=None):
        ...
        def wrap(request, *args, **kwargs):
            ...
            try:
                ...
                if user_rights is not None:
                    UserRights.objects.get(entity=entity, user=request.user, role__in=user_rights)
                else:
                    UserRights.objects.get(entity=entity, user=request.user)
            except:
                return ErrorResponse(403, AdminRightsError.USER_HAS_NO_RIGHT)

            # if the user tries to access an admin page, check that they're also part of the staff
            ...
            if entity.entity_type == Entity.EXTERNAL_ADMIN and external_rights is not None:
                try:
                    ExternalAdminRights.objects.get(entity=entity, right__in=external_rights)
                except:
