    ...
    def actual_decorator(function):
        ...
        def wrap(request, *args, **kwargs):
            ...
            if entity_id not in rights:
                return JsonResponse(
                    {"status": "forbidden", "message": "User has no rights to the requested entity"}, status=403
                )
            ...
            if role is not None:
                if isinstance(role, list):
                    if user_role not in role:
                        return JsonResponse(
                            {"status": "forbidden", "message": "Insufficient rights to the requested entity"},
                            status=403,
                        )
                elif role != rights.role:
                    return JsonResponse(
                        {"status": "forbidden", "message": "Insufficient rights to the requested entity"}, status=403
                    )
                else:
                    # all types of roles allowed
                    pass
