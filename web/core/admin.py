...
from core.models import LockedYear
...
class SustainabilityDeclarationAdmin(admin.ModelAdmin):
    ...
    def validate_declarations(self, request, queryset):
        for declaration in queryset:
            ...
            CarbureLot.objects.filter(period=period).filter(Q(carbure_client=declaration.entity) | Q(carbure_supplier=declaration.entity)).exclude(lot_status__in=[CarbureLot.DRAFT, CarbureLot.DELETED]).update(lot_status=CarbureLot.FROZEN)
    ...
    def invalidate_declarations(self, request, queryset):
        for declaration in queryset:
            period = declaration.period.year * 100 + declaration.period.month
            CarbureLot.objects.filter(period=period).filter(Q(carbure_client=declaration.entity) | Q(carbure_supplier=declaration.entity), lot_status=CarbureLot.FROZEN).update(lot_status=CarbureLot.ACCEPTED)
...
admin.site.register(LockedYear, LockedYearAdmin)
