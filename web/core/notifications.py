...
def notify_correction_request(lots):
    lots_by_supplier = (
        lots.select_related("carbure_client")
        .filter(carbure_supplier__isnull=False)
    ...
    for supplier in lots_by_supplier:
        try:
            notif = CarbureNotification.objects.get(
                dest_id=supplier["carbure_supplier"],
                type=CarbureNotification.CORRECTION_REQUEST,
                acked=False,
                email_sent=False,
...
def notify_correction_done(lots):
    lots_by_client = (
        lots.select_related("carbure_supplier")
        .filter(carbure_client__isnull=False)
    ...
    for client in lots_by_client:
        try:
            notif = CarbureNotification.objects.get(
                dest_id=client["carbure_client"],
                type=CarbureNotification.CORRECTION_DONE,
                acked=False,
                email_sent=False,
...
def notify_lots_rejected(lots):
    ...
    for supplier in lots_by_supplier:
        try:
            notif = CarbureNotification.objects.get(
                dest_id=supplier["carbure_supplier"],
                type=CarbureNotification.LOTS_REJECTED,
                acked=False,
                email_sent=False,
                meta__contains={"client": supplier["carbure_client__name"]},
            )
...
def notify_lots_received(lots):
    lots_by_client = (
        lots.filter(carbure_client__isnull=False)
        .select_related("carbure_supplier")
        .values("year", "carbure_client", "carbure_supplier__name")
    ...
    for client in lots_by_client:
        ...
        try:
            ...
            notif = CarbureNotification.objects.get(
                dest_id=client["carbure_client"],
                type=CarbureNotification.LOTS_RECEIVED,
                acked=False,
                email_sent=False,
                meta__contains={"supplier": client["carbure_supplier__name"]},
            )
...
def notify_lots_recalled(lots):
    ...
    for client in lots_by_client:
        try:
            notif = CarbureNotification.objects.get(
                dest_id=client["carbure_client"],
                type=CarbureNotification.LOTS_RECALLED,
                acked=False,
                email_sent=False,
                meta__contains={"supplier": client["carbure_supplier__name"]},
            )
