...
class StoredFileUploadToTest(TestCase):
    ...
    def test_upload_to_uses_entity_id_and_uuid(self):
        stored_file = StoredFile(entity_id=42)

        with patch("core.models.file.uuid4", return_value=UUID("123456781234123412341234567890ab")):
            path = stored_file_upload_to(stored_file, "Rapport.PDF")

        self.assertEqual(path, "files/42/123456781234123412341234567890ab.pdf")
    ...
    def test_upload_to_dir_overrides_default_directory(self):
        ...
        with patch("core.models.file.uuid4", return_value=UUID("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")):
            path = stored_file_upload_to(stored_file, "import.xlsx")

        self.assertEqual(path, "traceability/actions/7/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.xlsx")
    ...
    def test_save_fills_name_from_original_filename(self):
        ...
        self.assertTrue(stored_file.url.name.startswith(f"files/{entity.id}/"))
        self.assertTrue(stored_file.url.name.endswith(".pdf"))
