...
class EntityProducerFilter(FilterSet):
    """
    Generic FilterSet for biomethane models related to a producer.

    Override `producer_lookup` when the producer is reached through a relation
    (e.g. "digestate__producer_id" instead of "producer__id").

    Filter logic:
    - Producer: ?entity_id=123 → filters on producer_lookup=123
    - DREAL: ?entity_id=456&producer_id=789 → filters on producer_lookup=789
    """

    producer_lookup = "producer__id"
    ...
    def filter_by_entity(self, queryset, name, value):
        ...
        - Otherwise (Producer case), filter by producer_lookup = entity_id
        """
        ...
        return queryset.filter(**{self.producer_lookup: value})
