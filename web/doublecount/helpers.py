...
def load_dc_sourcing_file(entity, psite, user, filepath):
    ...
    try:
        ...
        dca, created = DoubleCountingAgreement.objects.get_or_create(producer=entity, production_site=psite, period_start=start, period_end=end)
...
def load_dc_production_file(entity, user, filepath):
    start = datetime.date(today.year + 1, 1, 1)
    end = datetime.date(today.year + 2, 12, 31)
    feedstocks = {f.code: f for f in MatierePremiere.objects.filter(is_double_compte=True)}
    countries = {f.code_pays: f for f in Pays.objects.all()}
...
def load_dc_recognition_file(entity, user, filepath):
