...
class EntityProductionSiteTest(TestCase):
    ...
    def test_change_user_role_missing_params(self):
        res = self.client.post(Endpoint.change_user_role, {"email": self.user.email, "entity_id": self.trader.id})
    ...
    def test_change_user_role_missing_user(self):
        res = self.client.post(
            Endpoint.change_user_role, {"email": "missing@carbure.test", "entity_id": self.trader.id, "role": "RW"}
        )
        self.assertEqual(res.status_code, 400)
        data = res.json()
    ...
    def test_change_user_role_no_prior_rights(self):
        ...
        res = self.client.post(
            Endpoint.change_user_role, {"email": other_user.email, "entity_id": self.trader.id, "role": "RW"}
        )
        self.assertEqual(res.status_code, 400)
        data = res.json()
    ...
    def test_change_user_role_ok(self):
        ...
        res = self.client.post(
            Endpoint.change_user_role, {"email": self.user.email, "entity_id": self.trader.id, "role": "RW"}
        )
