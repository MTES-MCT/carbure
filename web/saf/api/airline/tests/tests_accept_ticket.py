...
class SafTicketAcceptTest(TestCase):
    fixtures = [
        "json/biofuels.json",
        "json/feedstock.json",
        "json/countries.json",
    ...
    def test_reject_saf_ticket(self):
        ...
        response = self.client.post(reverse("saf-airline-accept-ticket"), query)

        assert response.status_code == 200
        assert response.json()["status"] == "success"

        assert SafTicket.objects.get(id=self.ticket.id).status == SafTicket.ACCEPTED
