...
from api.v4.helpers import filter_lots, filter_stock, get_entity_lots_by_status, get_lot_comments, get_lot_errors, get_lot_updates, get_lots_errors, get_lots_summary_data, get_lots_with_errors, get_lots_with_metadata, get_lots_filters_data, get_entity_stock
from api.v4.helpers import get_stock_events, get_stock_with_metadata, get_stock_filters_data, get_stocks_summary_data, get_transaction_distance
...
def get_snapshot(request, *args, **kwargs):
    ...
    alerts = lots.prefetch_related('genericerror_set').annotate(errors=Count('genericerror')).filter(errors__gt=0)
...
def get_lots(request, *args, **kwargs):
    ...
    try:
        lots = get_admin_lots_by_status(status)
        return get_lots_with_metadata(lots, None, request.GET)
    except Exception:
        traceback.print_exc()
...
def get_lots_summary(request, *args, **kwargs):
    ...
    try:
        lots = get_admin_lots_by_status(status)
        lots = filter_lots(lots, request.GET, None, will_aggregate=True)
...
def get_lot_details(request, *args, **kwargs):
    ...
    data['errors'] = get_lot_errors(lot)
...
def get_lots_filters(request, *args, **kwargs):
    ...
    lots = get_admin_lots_by_status(status)
    data = get_lots_filters_data(lots, request.GET, None, field)
    if data is None:
...
def get_admin_lots_by_status(status):
    ...
    if status == 'ALERTS':
        lots = get_lots_with_errors(lots, None, will_aggregate=True)
