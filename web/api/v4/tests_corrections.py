...
class LotsFlowTest(TestCase):
    ...
    def recall_lot(self, lot):
        response = self.client.post(reverse('api-v4-recall-lots'), {'entity_id': self.producer.id, 'lot_ids': [lot.id]})
        self.assertEqual(response.status_code, 200)
        lot = CarbureLot.objects.get(id=lot.id)
        return lot
    ...
    def test_simple_correction(self):
        ...
        self.assertEqual(lot.volume, 42000)
        # and mark as fixed
        response = self.client.post(reverse('api-v4-mark-as-fixed'), {'entity_id': self.producer.id, 'lot_ids': [lot.id]})
        self.assertEqual(response.status_code, 200)
        lot = CarbureLot.objects.get(id=lot.id)
