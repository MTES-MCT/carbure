...
def check_ghg_values(prefetched_data, lot, errors):
    ...
    if lot.feedstock in etd:
        ...
        if lot.etd > 2 * default_value and lot.etd > 5:
            errors.append(generic_error(error=CarbureMLGHGErrors.ETD_ANORMAL_HIGH, lot=lot, display_to_creator=False))
        if lot.country_of_origin:
            if not lot.country_of_origin.is_in_europe and lot.etd < default_value:
                errors.append(generic_error(error=CarbureMLGHGErrors.ETD_NO_EU_TOO_LOW, lot=lot, display_to_creator=False))
            if lot.country_of_origin.is_in_europe and lot.etd == default_value:
                errors.append(generic_error(error=CarbureMLGHGErrors.ETD_EU_DEFAULT_VALUE, lot=lot, display_to_creator=False))
    ...
    if lot.feedstock and lot.biofuel:
        ...
        if key in ep:
            ...
            if lot.ep < 0.8 * entry.average:
                errors.append(generic_error(error=CarbureMLGHGErrors.EP_ANORMAL_LOW, lot=lot, display_to_creator=False))
            if lot.ep > 1.2 * entry.default_value_max_ep:
                errors.append(generic_error(error=CarbureMLGHGErrors.EP_ANORMAL_HIGH, lot=lot, display_to_creator=False))
    ...
    if lot.delivery_date >= july1st2021:
        ...
        if lot.ghg_reduction_red_ii >= 100:
            ...
            errors.append(generic_error(error=CarbureSanityCheckErrors.GHG_REDUC_INF_50, lot=lot, is_blocking=True))
        else:
            # all good
            pass
    else:
    ...
    if lot.etd <= 0:
        errors.append(generic_error(error=CarbureSanityCheckErrors.GHG_ETD_0, lot=lot, is_blocking=True, field='etd'))
    if lot.ep <= 0:
        errors.append(generic_error(error=CarbureSanityCheckErrors.GHG_EP_0, lot=lot, is_blocking=True, field='ep'))
    if lot.el < 0:
    ...
    if lot.feedstock:
        ...
        if lot.feedstock.category == 'CONV' and lot.eec == 0:
            errors.append(generic_error(error=CarbureSanityCheckErrors.GHG_EEC_0, lot=lot, is_blocking=True, extra="GES Culture 0 pour MP conventionnelle (%s)" % (lot.feedstock.name), field='eec'))

        if lot.feedstock.category != 'CONV' and lot.feedstock.code != 'EP2' and lot.eec != 0:
            errors.append(generic_error(error=CarbureSanityCheckErrors.EEC_WITH_RESIDUE, lot=lot, field='eec'))
...
def check_certificates(prefetched_data, lot, errors):
    ...
    if lot.production_site_certificate:
        ...
        if cert not in prefetched_data['certificates']:
            errors.append(generic_error(error=CarbureCertificatesErrors.UNKNOWN_PRODSITE_CERT, lot=lot, display_to_recipient=True, field='production_site_certificate'))
        else:
    ...
    if not lot.supplier_certificate:
        errors.append(generic_error(error=CarbureCertificatesErrors.NO_SUPPLIER_CERT, lot=lot, display_to_recipient=True, field='supplier_certificate')) # blocking in check_mandatory_fields if client is an operator
    else:
    ...
    if lot.feedstock and lot.feedstock.is_double_compte:
        ...
        if not dc_cert:
            errors.append(generic_error(error=CarbureCertificatesErrors.MISSING_REF_DBL_COUNTING, lot=lot, display_to_recipient=True, field='dc_reference'))
        else:
            if dc_cert not in prefetched_data['double_counting_certificates']:
                ...
                if dcc_period < lot.period:
                    ...
                    # Non blocking
                    errors.append(generic_error(error=CarbureCertificatesErrors.EXPIRED_DOUBLE_COUNTING_CERTIFICATE, display_to_recipient=True, lot=lot))
                else:
                    pass
    return errors
...
def sanity_check(lot, prefetched_data):
    ...
    if lot.feedstock and lot.country_of_origin:
        ...
        if lot.feedstock.code == "RESIDUS_VINIQUES":
            errors.append(generic_error(error=CarbureSanityCheckErrors.DEPRECATED_MP, lot=lot, field='feedstock'))

        if lot.feedstock.code == 'SOJA':
            if lot.country_of_origin.code_pays not in ['US', 'AR', 'BR', 'UY', 'PY']:
                errors.append(generic_error(error=CarbureSanityCheckErrors.PROVENANCE_MP, lot=lot, extra="%s de %s" % (lot.feedstock.name, lot.country_of_origin.name), field='country_of_origin'))
    ...
    if lot.biofuel and lot.feedstock:
        ...
        if lot.biofuel.is_alcool and lot.feedstock.compatible_alcool is False:
            errors.append(generic_error(error=CarbureSanityCheckErrors.MP_BC_INCOHERENT, lot=lot, is_blocking=True, extra="%s issu de fermentation et %s n'est pas fermentescible" % (lot.biofuel.name, lot.feedstock.name), fields=['biofuel_code', 'feedstock_code']))
        if lot.biofuel.is_graisse and lot.feedstock.compatible_graisse is False:
            errors.append(generic_error(error=CarbureSanityCheckErrors.MP_BC_INCOHERENT, lot=lot, is_blocking=True, extra="Matière première (%s) incompatible avec Esthers Méthyliques" % (lot.feedstock.name), fields=['biofuel_code', 'feedstock_code']))
        ...
        if lot.feedstock.is_double_compte:
            ...
            if in_carbure_without_dc or not_in_carbure_without_dc:
                errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_REF_DBL_COUNTING, lot=lot, is_blocking=True, extra="%s de %s" % (lot.biofuel.name, lot.feedstock.name), field='production_site_dbl_counting'))
        ...
        if lot.feedstock.code in ['HUILES_OU_GRAISSES_ANIMALES_CAT1_CAT2', 'HUILES_OU_GRAISSES_ANIMALES_CAT3'] and lot.biofuel.code not in ['EMHA', 'HOE', 'HOG', 'HOC', 'HCC', 'HCG', 'HCE', 'B100']:
            errors.append(generic_error(error=CarbureSanityCheckErrors.MP_BC_INCOHERENT, lot=lot, is_blocking=True, extra="Des huiles ou graisses animales ne peuvent donner que des EMHA, B100 ou HOG/HOE/HOC", fields=['biofuel_code', 'feedstock_code']))
        if lot.feedstock.code == 'HUILE_ALIMENTAIRE_USAGEE' and lot.biofuel.code not in ['EMHU', 'HOE', 'HOG', 'HOC', 'HCC', 'HCG', 'HCE', "B100"]:
            errors.append(generic_error(error=CarbureSanityCheckErrors.MP_BC_INCOHERENT, lot=lot, is_blocking=True, extra="Des huiles alimentaires usagées ne peuvent donner que des EMHU, B100 ou HOG/HOE/HOC", fields=['biofuel_code', 'feedstock_code']))
    ...
    if lot.feedstock and lot.carbure_production_site:
        if lot.carbure_production_site.name in prefetched_data['my_production_sites']:
            ...
            if lot.feedstock not in mps:
                errors.append(generic_error(error=CarbureSanityCheckErrors.MP_NOT_CONFIGURED, lot=lot, display_to_recipient=False, field='feedstock_code'))
...
def sanity_check_mandatory_fields(lot):
    ...
    if not lot.volume:
        ...
        is_valid = False
    if not lot.biofuel:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_BIOFUEL, lot=lot, field='biofuel_code', is_blocking=True))
        is_valid = False
    if not lot.feedstock:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_FEEDSTOCK, lot=lot, field='feedstock_code', is_blocking=True))
        is_valid = False
    if lot.carbure_producer and lot.carbure_production_site is None:
        errors.append(generic_error(error=CarbureSanityCheckErrors.UNKNOWN_PRODUCTION_SITE, lot=lot, field='carbure_production_site', is_blocking=True))
        is_valid = False
    if not lot.carbure_production_site and not lot.production_site_commissioning_date:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_PRODUCTION_SITE_COMDATE, lot=lot, field='production_site_commissioning_date', is_blocking=True))
        is_valid = False

    if lot.delivery_type not in [CarbureLot.RFC, CarbureLot.FLUSHED] and lot.transport_document_reference is None:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_TRANSPORT_DOCUMENT_REFERENCE, lot=lot, field='transport_document_reference', is_blocking=True))
        is_valid = False
    if lot.delivery_type in [CarbureLot.BLENDING, CarbureLot.TRADING, CarbureLot.STOCK, CarbureLot.DIRECT, CarbureLot.UNKNOWN]:
        if lot.delivery_site_country and lot.delivery_site_country.code_pays == "FR":
            ...
            if not lot.carbure_delivery_site:
                errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_CARBURE_DELIVERY_SITE, lot=lot, field='carbure_delivery_site', is_blocking=True))
                is_valid = False
            # and we need to know the client
            if not lot.carbure_client:
                errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_CARBURE_CLIENT, lot=lot, field='carbure_client', is_blocking=True))
                is_valid = False
    if not lot.delivery_date:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_DELIVERY_DATE, lot=lot, field='delivery_date', is_blocking=True))
        is_valid = False
    if (lot.delivery_date - today) > datetime.timedelta(days=3650) or (lot.delivery_date - today) < datetime.timedelta(days=-3650):
        errors.append(generic_error(error=CarbureSanityCheckErrors.WRONG_DELIVERY_DATE, lot=lot, field='delivery_date', is_blocking=True))
        is_valid = False

    if not lot.delivery_site_country:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_DELIVERY_SITE_COUNTRY, lot=lot, field='delivery_site_country', is_blocking=True))
        is_valid = False
    if lot.delivery_site_country and lot.delivery_site_country.is_in_europe and not lot.country_of_origin:
        errors.append(generic_error(error=CarbureSanityCheckErrors.MISSING_FEEDSTOCK_COUNTRY_OF_ORIGIN, lot=lot, field='country_of_origin', is_blocking=True))
        is_valid = False
    ...
    if lot.carbure_client and lot.carbure_client.entity_type == Entity.OPERATOR:
        ...
        if not lot.supplier_certificate and not lot.vendor_certificate:
            errors.append(generic_error(error=CarbureCertificatesErrors.MISSING_SUPPLIER_CERTIFICATE, lot=lot, field='supplier_certificate', is_blocking=True))
            is_valid = False
