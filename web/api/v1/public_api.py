...
def depots_autocomplete(request):
    ...
    results = [{'value': '%s - %s - %s' % (i.name, i.depot_id, i.city), 'name': i.name, 'depot_id': i.depot_id,
                'city': i.city, 'country_code': i.country.code_pays, 'country_name': i.country.name} for i in depots]
    return JsonResponse({'suggestions': results})
