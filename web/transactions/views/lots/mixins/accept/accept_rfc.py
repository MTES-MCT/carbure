...
class AcceptRFCMixin:
    ...
    @action(methods=["post"], detail=False, url_path="accept-rfc")
    def accept_rfc(self, request, *args, **kwargs):
        entity_id = self.request.query_params.get("entity_id")

        lots = self.filter_queryset(self.get_queryset())
        # TODO: fix, required ?
        serializer = AcceptRFCSerializer(data=request.data)
