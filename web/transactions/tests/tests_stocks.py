...
class StocksFlowTest(TestCase):
    ...
    def test_get_stocks(self):
        ...
        response = self.client.get(
            reverse("v2-transactions-stocks-list") + f"?entity_id={self.producer.id}",
            query,
        )
        stocks_data = response.json()["results"]
        assert len(stocks_data) == 1
        assert stocks_data[0]["carbure_client"]["id"] == self.producer.id
    ...
    def test_get_stocks_summary(self):
        ...
        query = {"entity_id": self.producer.id}
        response = self.client.get(
            reverse("v2-transactions-stocks-summary") + f"?entity_id={self.producer.id}",
            query,
        )
    ...
    def test_get_stocks_details(self):
        ...
        response = self.client.get(
            reverse("v2-transactions-stocks-detail", kwargs={"id": stock.id}) + f"?entity_id={self.producer.id}",
            query,
        )
    ...
    def stock_split(self, payload, fail=False):
        response = self.client.post(
            reverse("v2-transactions-stocks-split") + f"?entity_id={self.producer.id}",
            {"entity_id": self.producer.id, "payload": payload},
            content_type="application/json",
        )
        # print("response : ", response.json(), response.status_code, {"entity_id": self.producer.id, "payload": payload})
        if not fail:
    ...
    def test_stock_split(self):
        ...
        response = self.client.post(
            reverse("v2-transactions-lots-delete") + f"?entity_id={self.producer.id}",
    ...
    def test_stock_flush(self):
        ...
        response = self.client.post(
            reverse("v2-transactions-stocks-flush") + f"?entity_id={self.producer.id}",
            query,
        )
        assert response.status_code == 200
    ...
    def test_stock_transformation(self):
        ...
        self.client.post(
            reverse("v2-transactions-stocks-transform") + f"?entity_id={self.producer.id}",
            body,
            content_type="application/json",
        )
        ...
        self.client.post(
            reverse("v2-transactions-stocks-cancel-transformation") + f"?entity_id={self.producer.id}",
            body,
        )
