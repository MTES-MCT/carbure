...
class LotsCertifMayhemTest(TestCase):
    ...
    def create_draft(self, lot=None, **kwargs):
        ...
        lot.update(kwargs)
        response = self.client.post(reverse("v2-transactions-lots-add") + f"?entity_id={self.producer.id}", lot)
        assert response.status_code == 200
        data = response.json()
        lot_id = data["id"]
    ...
    def send_lot(self, lot, expected_status=200):
        response = self.client.post(
            reverse("v2-transactions-lots-send") + f"?entity_id={self.producer.id}",
