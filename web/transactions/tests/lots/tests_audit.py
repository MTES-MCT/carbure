...
class LotsAuditTest(TestCase):
    ...
    def test_mark_conform(self):
        ...
        response = self.client.post(
            reverse("v2-transactions-lots-mark-conform") + f"?entity_id={self.auditor.id}",
            {"selection": [lot.id]},
        )
    ...
    def test_mark_non_conform(self):
        lot = self.create_draft()
        response = self.client.post(
            reverse("v2-transactions-lots-mark-non-conform") + f"?entity_id={self.auditor.id}",
            {"selection": [lot.id]},
        )
        assert response.status_code == 200
        lot.refresh_from_db()
    ...
    def test_toggle_pin_audit(self):
        lot = self.create_draft()
        response = self.client.post(
            reverse("v2-transactions-lots-toggle-pin") + f"?entity_id={self.auditor.id}",
