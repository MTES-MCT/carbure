...
@transaction.atomic
def replace_charge_point_id(cpo: Entity, file: Union[File, str], apply: bool = True):
    try:
    ...
    if len(cols) < 2:
        raise ParseError("Excel file must contain at least two columns.")
    ...
    pdcs = ElecChargePoint.objects.filter(cpo__name__icontains=cpo, charge_point_id__in=mapping_id.keys(), is_deleted=False)
    ...
    return updated, deleted, not_found
